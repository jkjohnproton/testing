...
$pageTitle			= (($clsaxUserMedicalReport->userId == "")?"Add":"Edit")." a UserMedicalReport in ".getProductName();	
$metaDescription	= "";
$metaKeywords		= "";
?>
<!DOCTYPE html>
...

{
	...
	$clsaxUserMedicalReport->returnUrl = "axUserMedicalReport.php";
	
	$clsaxUserMedicalReport->userId = $_SESSION["AXUSERID"];
	
	$clsaxUserMedicalReport->postUserMedicalReport();
	
	$medicationList	= HTMLOptionKeyValArray($arrMedication,$clsaxUserMedicalReport->medicationName);
	
	$familyDiseaseList	= HTMLOptionKeyValArray($arrFamilyDisease,$clsaxUserMedicalReport->familyDisease);
	
	$exerciseList	= HTMLOptionKeyValArray($arrExercise,$clsaxUserMedicalReport->exercise);
	
	$smokingList	= HTMLOptionKeyValArray($arrSmoking,$clsaxUserMedicalReport->smoking);
	
	$drinkingList	= HTMLOptionKeyValArray($arrDrinking,$clsaxUserMedicalReport->alcohol);
	
	$dietList	= HTMLOptionKeyValArray($arrDiet,$clsaxUserMedicalReport->diet);
	
	$arrPatient = array();   
	$clsaxPatient->userId = $_SESSION["AXUSERID"];
	$arrPatient = $clsaxPatient->retrievePatientArray();
	$patientList = HTMLOption2DArray($arrPatient,"patientId","patientName",$clsaxUserMedicalReport->patientId);
	...
	$clsaxUserMedicalReport->returnUrl	 = $clsaxUserMedicalReport->returnUrl;
	include_once($includePath."templates/axUserMedicalReport.tmpl.php");
