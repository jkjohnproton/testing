...
$pageTitle			= "List of Diseases in ".getProductName();
$metaDescription	= "";
$metaKeywords		= "";
?>
<!DOCTYPE html>
...
function writeMain($connect, $includePath) 
{
	...
	$clsaxDisease	->	sortDirection = "DESC";
	$clsaxDisease	->	postDisease();
	$pageRange				= 	HTMLOptionKeyValArray($arrPageRange,$clsaxDisease->pageSize);
	$arrDisease			= 	array();
	$clsaxDisease->submitted = 1;
	if($clsaxDisease->submitted==1)
	{
		$arrDisease		= 	$clsaxDisease->retrieveDiseaseArray();
	}
	$frmControlStatus 	= "";
	if (count($arrDisease) == 0 ) 
	{
	...
	$extraParameters	.="diseaseName=$clsaxDisease->diseaseName&";

	$extraParameters	.= "pageSize=$clsaxDisease->pageSize&submitted=$clsaxDisease->submitted";
	
	include_once($includePath."templates/axDiseaseList.tmpl.php");
}
